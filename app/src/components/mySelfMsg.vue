		...
		<div class="box">
			...
			<div>
				<van-cell-group inset v-for="(item,index) in formMsg" :key="item.label">
				  <van-field v-model="item.value" disabled :label="item.label"  />
				</van-cell-group>
			</div>
			<br>
			<div>
		...
		methods:{
			...
			async getMsg(){
				...
				if(res.status==200){
					...
					this.formMsg[1].value=res.data.email
					this.formMsg[2].value=res.data.sex
					this.formMsg[3].value=res.data.phone
					this.formMsg[4].value=res.data.address
					this.formMsg[5].value=res.data.username
					this.url1=res.data.headphoto
				}
